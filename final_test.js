...
const paddockManagers = [
  ...
  /*
    SECCIÓN PROBLEMAS
      - debe ejecutarse sin errores con: node nombre-apellido.js
  ...
  */
  ...
  function listPaddockManagersByName() {
      ...
      return paddockManagers.sort((nom1, nom2)=>nom1.name-nom2.name).map(ruts=>ruts.taxNumber)
  ...
  /*   
    preciototal=paddocks.reduce((t,i)=>t+i.area,0);
   const filtrado1=paddocks.filter(tipo=>tipo.paddockTypeId==1).map(tota1=>tota1.area).reduce((a,v)=>a+v,0); 
 */
  const total={}
  paddocks.forEach(pad=>{const{paddockTypeId, area}=pad
    total[paddockTypeId]??=0
    total[paddockTypeId]+=area
       })

  return paddockType.map(var1=>({...var1,sum:total[var1.id]})).sort((a, b)=>b.sum-a.sum).map(x=>x.name)
  ...
  return paddockManagers.map(var1=>({...var1,sum:total[var1.id]})).sort((a, b)=>b.sum-a.sum).map(x=>x.name) 
  ...
  function farmManagerNames() {
   ...
   const personas=paddocks.reduce((personas, padd)=>{
          const campos=farms.find(campo=>campo.id===padd.farmId).name
          const idd=paddockManagers.find(nom=>nom.id===padd.paddockManagerId)

          personas.hasOwnProperty(campos)?
            personas[campos].includes(idd)?null:
            personas[campos].push(idd):
            personas[campos]=[]
        ...
        for (const campo in personas) {
          personas[campo]=personas[campo].sort((nom1, nom2)=>nom1.name-nom2.name).map(idd=>idd.taxNumber)
        }
  ...
  function biggestAvocadoFarms() {
    ...
    const fruta=paddockType.filter(a=>a.name==="PALTOS")[0].id
      
      return paddocks.filter(are=>are.paddockTypeId===fruta&&are.area>=20000).sort((a,b)=>b-a).map(are=>are.area)
  ...
  function biggestCherriesManagers() {
    ...
    const campoid=farms.filter(campo=>campo.name==="FORESTAL Y AGRICOLA LO ENCINA")[0].id
    const frutaid=paddockType.filter(fru=>fru.name==="CEREZAS")[0].id
    const total=paddocks.filter(padd=>padd.farmId===campoid&&padd.area>1000&&padd.paddockTypeId===frutaid)[0].paddockManagerId
    
    return  paddockManagers.filter(are=>are.id===total).sort((a,b)=>a-b).map(are=>are.name)
  ...
  function farmManagerPaddocks() {
          ...
          const personas=paddocks.reduce((personas, cont)=>{
          const nombres=paddockManagers.find(nom=>nom.id===cont.paddockManagerId).name
          const campos=farms.find(campos=>campos.id===cont.farmId)

          personas.hasOwnProperty(nombres)?
            personas[nombres].includes(campos)?null:
            personas[nombres].push(campos):
            personas[nombres]=[]
        ...
        for (const nom in personas){
         personas[nom]=personas[nom].map(nom=>nom.name).sort((a,b)=>a.name-b.name)
        }
  ...
  function paddocksManagers() {
      ...
      const personas=paddocks.reduce((personas, cont)=>{
      const cuartel=paddockType.find(campos=>campos.id===cont.paddockTypeId).name
      const años=paddocks.find(año=>año.id===cont.harvestYear)
      const nombres=paddockManagers.find(nom=>nom.id===cont.paddockManagerId)
      const con=cuartel.concat('-',años)

      personas.hasOwnProperty(con)?
        personas[con].includes(nombres)?null:
        personas[con].push(nombres):
        personas[con]=[]
      ...
      return personas
       ...
       for (const nom in personas){
      personas[nom]=personas[nom].sort((nom1, nom2)=>nom1.name-nom2.name).map(nom=>nom.name)
      .concat(personas[nom]=personas[nom].sort((nom1, nom2)=>nom1.name-nom2.name).map(nom=>nom.taxNumber))
  ...
  function newManagerRanking() {
      ...
      const padd= paddockManagers
      padd.push({id: 7, taxNumber: '99999999', name: 'KEVIN MORA' })

      const cuartel = paddocks
      cuartel.push({paddockManagerId:7, farmId:1, paddockTypeId:4, harvestYear:2017, area:900})

      return sortFarmManagerByAdminArea(cuartel).findIndex(a=>a==='KEVIN MORA') //devolver al ranking usando pregunta 3
